...
namespace HttpTwo
{
    public class Http2Client
    {
        ...
        public async Task<Http2Response> PostAsync(Uri uri, NameValueCollection headers = null, byte[] data = null)
        {
            return await SendAsync(uri, HttpMethod.Post, headers, data).ConfigureAwait(false);
        }

        public async Task<Http2Response> PostAsync(Uri uri, NameValueCollection headers = null, Stream data = null)
        {
            return await SendAsync(uri, HttpMethod.Post, headers, data).ConfigureAwait(false);
        }

        public async Task<Http2Response> SendAsync(Uri uri, HttpMethod method, NameValueCollection headers = null, byte[] data = null)
        {
            ...
            return await SendAsync(new CancellationToken(), uri, method, headers, ms).ConfigureAwait(false);
        }

        public async Task<Http2Response> SendAsync(Uri uri, HttpMethod method, NameValueCollection headers = null, Stream data = null)
        {
            return await SendAsync(new CancellationToken(), uri, method, headers, data).ConfigureAwait(false);
        }

        public async Task<Http2Response> SendAsync(CancellationToken cancelToken, Uri uri, HttpMethod method, NameValueCollection headers = null, Stream data = null)
        {
            ...
            for (int i = 0; i < numFrames; i++)
            {
                ...
                Array.Copy(headerData, i * (int)maxFrameSize, frame.HeaderBlockFragment, 0, (int)amt);
